...
const BackgroundWave = () => (
  <div className="absolute inset-0 overflow-hidden z-0"> 
    <svg viewBox="0 0 1440 500" xmlns="http://www.w3.org/2000/svg" className="w-full h-full" preserveAspectRatio="none">
      <path 
...
const GetStarted = () => {
  const { registerUser, verifyOtp, loginUser } = useContext(AppContext);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (isSignUp && !showOtpInput) {
        ...
        if (res.success) {
          toast.success(res.message + " You can now login.");
          setIsSignUp(false); 
          setShowOtpInput(false);
          setOtp('');
          navigate('/'); 
        } else toast.error(res.message || "OTP verification failed");
      } else {
        // Login
          ...
          <div className="w-full max-w-md">
            <div className="bg-white/80 backdrop-blur-sm p-8 rounded-2xl shadow-lg">
              <h2 className="text-3xl font-bold text-center text-charcoal mb-6">
                {isSignUp ? (showOtpInput ? 'Verify OTP' : 'Create Account') : 'Welcome Back'}
              </h2>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {isSignUp && showOtpInput && (
                  <div>
                    <label htmlFor="otp" className="block text-sm font-medium text-slate-gray">OTP</label>
                    <input
                      id="otp"
                      type="text"
                      required
                      ...
                      <button
                        ...
                        className={`w-full flex justify-center py-3 px-4 rounded-md shadow-sm text-sm font-medium 
                          ${resendTimer > 0 
                            ...
                            : "text-primary bg-white hover:bg-slate-100 focus:ring-2 focus:ring-primary transition-colors"}
                        `}
                      >
              ...
              <p className="mt-6 text-center text-sm text-slate-gray">
                {isSignUp
                  ? (showOtpInput ? null : 'Already have an account?')
                  : "Don't have an account?"}
                <button
                  onClick={() => { setIsSignUp(!isSignUp); setShowOtpInput(false); }}
                  className="ml-1 font-medium text-primary-orange hover:text-orange-600 focus:outline-none"
                >
